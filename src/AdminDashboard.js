...
import {
  ...
  getReporte, getMisAsistenciasDeUsuario, getPagosExtraDeUsuario,
...
export default function AdminDashboard() {
  ...
  const [inacUsuarios,  setInacUsuarios]  = useState([]);
  const [almacenes,     setAlmacenes]     = useState([]);
  const [inacAlmacenes, setInacAlmacenes] = useState([]);
  const [reporte,       setReporte]       = useState([]);
  const [historial,     setHistorial]     = useState([]);
  const [pagosExtraTrab,setPagosExtraTrab]= useState([]);
  const [loadingHist,   setLoadingHist]   = useState(false);
  const [tabHist,       setTabHist]       = useState("asistencias");
  ...
  const verHistorial = async (usuario) => {
    setTrab(usuario); setVista(VISTA.HISTORIAL);
    setLoadingHist(true); setTabHist("asistencias");
    try {
      const [asist, pagos] = await Promise.all([
        getMisAsistenciasDeUsuario(token, usuario.id),
        getPagosExtraDeUsuario(token, usuario.id),
      ]);
      setHistorial(asist || []);
      setPagosExtraTrab(pagos || []);
    } catch { setHistorial([]); setPagosExtraTrab([]); }
    finally { setLoadingHist(false); }
  };

  const handlePagoCompleto = async (uid) => {
    if (!window.confirm("¿Registrar pago completo? Las asistencias pasarán a PAGADO y el trabajador recibirá una notificación.")) return;
    try {
  ...
  const exportarExcel = () => {
    ...
    const asistDatos = historial.map(a => ({
      Fecha: a.fecha, "Almacén": a.almacen?.nombre || "—",
      "Pago (S/.)": Number(a.pagoDia).toFixed(2), Estado: a.estado || "—",
    }));
    asistDatos.push({}, { Fecha: "Subtotal asistencias", "Pago (S/.)": Number(rep?.totalGanado || 0).toFixed(2) });
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(asistDatos), "Asistencias");

    const pagosDatos = pagosExtraTrab.map(p => ({
      Fecha: p.fecha, Motivo: p.motivo, "Monto (S/.)": Number(p.monto).toFixed(2),
    }));
    pagosDatos.push({}, { Fecha: "Subtotal pagos extra", "Monto (S/.)": Number(rep?.pagosExtra || 0).toFixed(2) });
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(pagosDatos), "Pagos Extra");

    const resumen = [
      { Concepto: "Total asistencias",    "Monto (S/.)": Number(rep?.totalGanado || 0).toFixed(2) },
      { Concepto: "Pagos extra",          "Monto (S/.)": `+ ${Number(rep?.pagosExtra || 0).toFixed(2)}` },
      { Concepto: "Adelantos descontados","Monto (S/.)": `- ${Number(rep?.adelantos || 0).toFixed(2)}` },
      { Concepto: "NETO A PAGAR",         "Monto (S/.)": Number(rep?.neto || 0).toFixed(2) },
    ];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(resumen), "Resumen");
    XLSX.writeFile(wb, `reporte_${trab.usuario}.xlsx`);
  };

  useEffect(() => { if (!token) { window.location.href = "/"; return; } cargarDatos(); }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // ─── VISTA HISTORIAL ─────────────────────────────────
  if (vista === VISTA.HISTORIAL && trab) {
    const rep = reporte.find(r => r.usuario === trab.usuario);
    const totalPagosExtraTrab = pagosExtraTrab.reduce((a, p) => a + (p.monto || 0), 0);
    return (
      <div style={s.page}>
        <div style={s.header}>
          <div style={s.headerLeft}>
            <button onClick={() => setVista(VISTA.TRABAJADORES)} style={s.btnVolver}>← Volver</button>
            <div><h1 style={s.headerTitle}>👷 {trab.usuario}</h1><p style={s.headerSub}>Historial completo</p></div>
          </div>
        ...
        <div style={s.card}>
          <div style={s.tabBar}>
            <button onClick={() => setTabHist("asistencias")} style={tab(tabHist === "asistencias")}>
              📅 Asistencias ({historial.length})
            </button>
            <button onClick={() => setTabHist("extras")} style={tab(tabHist === "extras")}>
              💰 Pagos extra ({pagosExtraTrab.length})
            </button>
          </div>

          {loadingHist ? <p style={s.empty}>Cargando...</p> : (
            <>
              {tabHist === "asistencias" && (
                historial.length === 0 ? <p style={s.empty}>Sin asistencias.</p> : (
                  <table style={s.table}>
                    <thead><tr><th style={s.th}>Fecha</th><th style={s.th}>Almacén</th><th style={s.th}>Pago</th><th style={s.th}>Estado</th></tr></thead>
                    <tbody>
                      {historial.map((a, i) => (
                        <tr key={i} style={i % 2 === 0 ? s.trEven : {}}>
                          <td style={s.td}>{a.fecha}</td>
                          <td style={s.td}>{a.almacen?.nombre || "—"}</td>
                          <td style={{ ...s.td, color: a.estado === "PAGADO" ? "#9ca3af" : "#374151", textDecoration: a.estado === "PAGADO" ? "line-through" : "none" }}>
              ...
              {tabHist === "extras" && (
                pagosExtraTrab.length === 0 ? <p style={s.empty}>Sin pagos extra.</p> : (
                  <table style={s.table}>
                    <thead><tr><th style={s.th}>Fecha</th><th style={s.th}>Motivo</th><th style={s.th}>Monto</th></tr></thead>
                    <tbody>
                      {pagosExtraTrab.map((p, i) => (
                        <tr key={i} style={i % 2 === 0 ? s.trEven : {}}>
                          <td style={s.td}>{p.fecha}</td>
                          <td style={s.td}>{p.motivo}</td>
                          <td style={{ ...s.td, color: "#16a34a", fontWeight: 600 }}>+ S/. {Number(p.monto).toFixed(2)}</td>
                        </tr>
                      ))}
                      <tr style={{ background: "#f0fdf4" }}>
                        <td colSpan={2} style={{ ...s.td, fontWeight: 700, color: "#166534" }}>Total pagos extra</td>
                        <td style={{ ...s.td, fontWeight: 700, color: "#166534" }}>S/. {totalPagosExtraTrab.toFixed(2)}</td>
                      </tr>
                    </tbody>
                  </table>
                )
                  ...
                  <div key={u.id} style={s.trabCard}>
                    <div style={s.trabInfo}>
                      ...
                      <div>
                        ...
                        <p style={s.trabMeta}>
                          {rep ? `${rep.diasTrabajados} días · S/. ${Number(rep.neto).toFixed(2)} neto` : "Sin asistencias"}
                        </p>
                      </div>
                    </div>
    ...
    return (
      <div style={s.page}>
        ...
        <div style={s.card}>
          ...
          {inacUsuarios.length === 0 ? <p style={s.empty}>Ninguno.</p> : (
        ...
        <div style={s.card}>
          ...
          {inacAlmacenes.length === 0 ? <p style={s.empty}>Ninguno.</p> : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {inacAlmacenes.map(a => (
                <div key={a.id} style={s.listItem}>
                  <div>
                    <span style={s.listName}>{a.nombre}</span>
                    {a.ubicacion && <span style={s.listSub}> · {a.ubicacion}</span>}
                    <span style={s.listSub}> · S/. {a.pagoBase}</span>
                  </div>
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.statsRow}>
        ...
        <StatBox label="Neto total reporte"   valor={`S/. ${reporte.reduce((a, r) => a + (r.neto || 0), 0).toFixed(2)}`} emoji="💰" color="#d97706" />
      </div>
          ...
          <table style={s.table}>
            <thead>
              <tr>
                ...
                <th style={s.th}>Días pend.</th>
                <th style={s.th}>Total asist.</th>
                <th style={s.th}>Pagos extra</th>
                <th style={s.th}>Adelantos</th>
                <th style={s.th}>Neto a pagar</th>
              </tr>
            </thead>
            <tbody>
              {reporte.map((r, i) => (
