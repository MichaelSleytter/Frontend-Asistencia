...
import {
  getMisAsistencias, getMiTotal, getMisPagosExtra,
  getAlmacenesPublic, registrarAsistencia, registrarPagoExtra,
...
export default function UserDashboard() {
  ...
  const cargarDatos = async () => {
    try {
      const [asist, tot, pagos, notifs] = await Promise.all([
  ...
  const handleConfirmar = async (id) => {
    setConfirmando(id);
    try { await confirmarNotif(token, id); await cargarDatos(); }
  ...
  // ─── PANTALLA DE NOTIFICACIONES PENDIENTES ───────────
        ...
        <div style={s.card}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 13, color: "#888" }}>Confirma para continuar usando el panel</p>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {notificaciones.map(n => (
              <div key={n.id} style={notifCard(n.tipo)}>
                <span style={{ fontSize: 26 }}>{n.tipo === "PAGO_COMPLETO" ? "✅" : "💵"}</span>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: "0 0 4px", fontSize: 14, color: "#374151" }}>{n.mensaje}</p>
                  <p style={{ margin: 0, fontSize: 12, color: "#9ca3af" }}>
                    {new Date(n.fechaCreacion).toLocaleDateString("es-PE", { day: "2-digit", month: "long", year: "numeric" })}
                  </p>
                </div>
                <button onClick={() => handleConfirmar(n.id)} disabled={confirmando === n.id} style={s.btnConfirmar}>
    ...
    <div style={s.page}>
      ...
      <div style={s.totalCard}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 8 }}>
          <span style={s.totalChip}>📅 {diasPendientes} día{diasPendientes !== 1 ? "s" : ""}</span>
          <span style={s.totalChip}>💰 S/. {Number(totalPagosExtra).toFixed(2)} en extras</span>
        </div>
      </div>
      ...
      <div style={s.grid}>
        ...
        <div style={s.card}>
          ...
          <input style={s.input} placeholder="Ej: Hora extra, bono, refrigerio..." value={motivo} onChange={e => setMotivo(e.target.value)} />
      ...
      <div style={s.card}>
        <div style={s.tabBar}>
          <button onClick={() => setTabActiva("asistencias")} style={tab(tabActiva === "asistencias")}>
            📅 Asistencias ({asistencias.length})
          </button>
          <button onClick={() => setTabActiva("extras")} style={tab(tabActiva === "extras")}>
            💰 Pagos extra ({pagosExtra.length})
          </button>
        </div>

        {tabActiva === "asistencias" && (
          asistencias.length === 0 ? <p style={s.empty}>No hay asistencias registradas.</p> : (
            <table style={s.table}>
              <thead>
                <tr>
        ...
        {tabActiva === "extras" && (
          pagosExtra.length === 0 ? <p style={s.empty}>No hay pagos extra registrados.</p> : (
            <table style={s.table}>
              <thead>
                <tr>
                  <th style={s.th}>Fecha</th>
              ...
              <tbody>
                ...
                <tr style={{ background: "#f0fdf4" }}>
                  ...
                  <td style={{ ...s.td, fontWeight: 700, color: "#166534" }}>S/. {totalPagosExtra.toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          )
...
const s = {
  ...
  totalChip:  { background: "rgba(255,255,255,0.2)", borderRadius: 99, padding: "3px 12px", fontSize: 13 },
